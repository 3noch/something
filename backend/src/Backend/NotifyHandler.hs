...
notifyHandler _runTransaction msg vs = case _dbNotification_message msg of
  Notification_Tag :=> Identity (_change, TagOccurrence tagName translationId (startRef, startWord) (endRef, endWord)) ->
    let
      entry = MMap.singleton translationId $ MMap.singleton tagName $ MMap.singleton (startRef, startWord, endRef, endWord) ()
    in pure mempty
